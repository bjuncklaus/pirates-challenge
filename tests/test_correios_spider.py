...
class TestCorreiosSpider(unittest.TestCase):
    ...
    def test_is_uf_table_with_no_uf_selected_should_return_false(self):
        correios = CorreiosSpider()
    ...
    def test_yield_result_should_yield_same_uf_item(self):
        correios = CorreiosSpider()

        expected_ufloader = UfItemLoader(selector=Selector(text=''))
        uf = 'uf'
        ...
        correios.created_ufs[uf] = expected_ufloader
        for actual_uf_item in correios.yield_result(None, Selector(text=''), 'uf'):
            self.assertEqual(expected_ufloader.load_item(), actual_uf_item)
